...
- `drawPiece` が 1 つのコマを描く。当たり判定 `pieceHitBox` は
  **`drawPiece` と同じ式（`pt` `ry` `fh` など）から導いている**ので、片方を変えたらもう片方も直す。
  HP メーター（プレイヤー色のまるぽち＋横棒）の寸法だけは `hpBarGeom()` に切り出して
  両方から呼んでいる。ここが絵と判定でいちばんずれやすかった。
...
- **隊長の円盤だけ 1.13 倍で描いていた**（修正済み）。物理は `stepPhysics()` で全コマ一律 `R`
  （`d >= R * 2`／`ARENA.r - R`／`P.r + R`）なのに、`drawPiece()` と `pieceHitBox()` だけ
  隊長を `boss = 1.13` で拡大していた。見えている円盤の縁を狙うとスカる。
  **絵の半径は `R` そのもの**にして、隊長は冠・天面の輪・HP150 で見分ける。
- **`R` を上げると初期配置が食い込む**（対処済み）。`makePieces()` は駒を陣営ごとに
  1 本の弧へ並べるので、人数が多く、かつその向きの盤面が狭いと隣と `2R` に届かない。
  R=50 では氷湖の四角の 7・8 人戦で重なった。角度差から必要な配置半径 `rMin` を出して
  外へ押し出し、壁にめり込まない範囲でクランプしている。`R` を触ったら全ステージ×
  2〜8 人で最小コマ間距離を測り直すこと。
